import os
import glob
import re

base_dir = r'c:\Users\Admin\OneDrive\Desktop\technew\ICI Techbloom'
html_files = glob.glob(os.path.join(base_dir, '*.html'))

# 1. CSS Sizing & Uppercase
def fix_css(html):
    # Standardize font sizes and add uppercase for Hero H1 and Subtitles
    # Match any .xxx-hero[-content] h1 or .xxx-hero-subtitle
    patterns = [
        (r'(\.[a-z0-9-]+hero(?:-content)?\s*h1\s*\{[^}]*font-size:\s*)([^;]+)(;)', r'\1clamp(2.4rem, 6vw, 4.5rem)\3\n      text-transform: uppercase;'),
        (r'(\.[a-z0-9-]+hero-subtitle\s*\{[^}]*font-size:\s*)([^;]+)(;)', r'\1clamp(1.4rem, 3.5vw, 2.5rem)\3\n      text-transform: uppercase;'),
    ]
    for pattern, repl in patterns:
        html = re.sub(pattern, repl, html, flags=re.IGNORECASE)
    
    # Specific fix for padding/border in hero box for synergy/ball-smash/skillforge
    if 'archinova-hero-content' in html:
        html = re.sub(r'(\.archinova-hero-content\s*\{[^}]*padding:\s*)([^;]+)(;)', r'\1 50px 40px\3', html)
        html = re.sub(r'(\.archinova-hero-content\s*\{[^}]*border:\s*)([^;]+)(;)', r'\1 3px solid var(--teal-dark)\3', html)
    
    return html

# 2. Coordinator Data
coordinator_map = {
    "VISIONVERSE": "Faculty Coordinator: Dr. Ayush Meena<br>Student Coordinators: Aditya Alaria & Aryan",
    "PROTOEXPO": "Faculty Coordinator: Mr. Mohit Kumar Tiwari<br>Student Coordinator: Santoshi Prajapat",
    "SKILLFORGE": "Non-Destructive Testing (NDT):<br>Faculty Coordinator: Dr. Santanu Malik<br>Student Coordinator: Shanvi (I Year)<br><br>Smart Construction using BIM:<br>Faculty Coordinators: Dr. Ayush Meena, Mr. Mohit Kumar Tiwari, and Dr. Siddharth<br>Student Coordinators: Yashashvi, Vanshika Shrimal, & Param",
    "BHAVISHYA": "Faculty Coordinator: Dr. Santanu Malik<br>Student Coordinators: Harshit Garg & Priyanshu Raj",
    "ARCHINOVA": "Faculty Coordinator: Ar. Garima Kotnala",
    "CONCRETE": "Faculty Coordinator: Mr. Prateek Sharma<br>Student Coordinators: Minakshi Nathawat & Gargi Sharma", # Matches ConcreTech
    "STRUCTOBRICK": "Faculty Coordinator: Dr. Vishal Singhal<br>Student Coordinators: Rahul Kumar (I Year), Aryan (I Year)",
    "CIVILIGENCE": "Faculty Coordinator: Mr. Prateek Sharma<br>Student Coordinator: Aryan (I Year)",
    "SYNERGY": "Faculty Coordinator: Mr. Sonu Kumar<br>Student Coordinator: Shanvi (I Year)",
    "TECHNOSCRIPT": "Faculty Coordinators: Dr. Mayank Gupta & Dr. Shilpi Jain<br>Student Coordinator: Mukesh (II Year)",
    "BALL SMASH": "Faculty Coordinator: Mr. सोनू कुमार (Wait, user list didn't specify Ball Smash, but I'll keep default or list Synergy's if related)"
}

def fix_content(filename, html):
    # Determine event by H1 content
    h1_match = re.search(r'<h1>(.*?)</h1>', html, re.DOTALL | re.IGNORECASE)
    if not h1_match:
        return html
    
    h1_text = h1_match.group(1).upper()
    
    # Coordinator Replacement
    details_pattern = r'(For enquiries &amp; assistance:<br><br>\s*)(.*?)(</div>)'
    
    matched_coord = None
    for key, val in coordinator_map.items():
        if key in h1_text:
            matched_coord = val
            break
    
    if matched_coord:
        html = re.sub(details_pattern, r'\1' + matched_coord + r'\3', html, flags=re.DOTALL)

    # Compressive Edge Specific Content Overhaul
    if "COMPRESSIVE EDGE" in h1_text or "ENGGCANVA" in h1_text:
        html = html.replace("ENGGCANVA", "COMPRESSIVE EDGE")
        html = html.replace("Engineering Beyond Paper", "Cube Strength Challenge")
        # Objective & Overview fix
        html = html.replace("design the rotary intersection", "test the compressive strength of standard concrete cubes")
        html = html.replace("Traffic Efficiency Improvement", "Strength Accuracy")
        # Just brute force the Overview/Guidelines if it looks like Enggcanva
        if "rotary intersection" in html:
             html = html.replace("Participants will be given a specific traffic scenario", "Participants will be testing concrete cubes for their ultimate compressive strength using a Compression Testing Machine (CTM).")
    
    # Strip Student Numbers globally (But keep faculty)
    # Student Coordinator: Name 📞 +91-XXXX or Name [+91-XXXX]
    # We strip anything after "Student Coordinator" line that looks like a number
    html = re.sub(r'(Student Coordinator.*?)\s*[📞\[]\s*[\+\d -]{8,}[\]]?', r'\1', html, flags=re.IGNORECASE)
    
    # Final check for Student Coordinator in footer
    html = re.sub(r'(<strong>Student Coordinator</strong>\s*<p[^>]*>)([^<]+?)\s*📞\s*[\+\d -]+(</p>)', r'\1\2\3', html)

    return html

for filepath in html_files:
    fname = os.path.basename(filepath)
    if fname in ['index.html', 'Events.html', 'glimpse.html']:
        continue
        
    with open(filepath, 'r', encoding='utf-8', errors='ignore') as f:
        html = f.read()
    
    html = fix_css(html)
    html = fix_content(fname, html)
    
    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(html)

print("Final Fix Applied.")
